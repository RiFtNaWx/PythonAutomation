...
class DataLogger:
    ...
    def __init__(self, filename=None, test_name="default", user_id="unknown"):
        if filename is None:
            ...
            self.filename = f"{self.runID}_datalog.csv"
        else:
    ...
    def save_to_csv(self):
        """Save all logged entries to CSV file (Excel-compatible)."""
        if not self.entries:
            return
        ...
        if functional_fails > 0:
            ...
            soft_bin = 2  # Prioritize functional fail (closer to 1)
        elif ac_fails > 0:
            overall_pass_fail = 'FAIL'
            soft_bin = 3
        else:
        ...
        if os.path.exists(self.filename):
            # Append only new result rows when file already exists.
            df_results.to_csv(self.filename, mode='a', index=False, header=False, encoding='utf-8')
        else:
