...
def _settle_loop_ok() -> list[str]:
    ...
    vcc_src = _fn_src(src, "_vcc_corners")
    force_src = _fn_src(src, "_force_psu")
    if "params" not in vcc_src or "vcc_list" not in vcc_src:
        errors.append("_vcc_corners must honor RunParams.vcc_list overlay")
    if "* 1.1" in power_src or "* 1.1" in force_src:
        errors.append("PSU OVP must not use *1.1 (+0.5) -- 5.5 V -> 6.55 trips ceiling 6.0")
    if "int(ch) == 3" not in force_src and "ch == 3" not in force_src:
        errors.append("_force_psu must clamp DP832 CH3 at 5.0 V so 5.5 V still enables")
    if "time.sleep" in power_src:
        errors.append("_power_vcc must not sleep-as-settle; caller uses settle-to-stable")
    if "vcc_awg" not in power_src or "set_output_load" not in power_src:
        errors.append("_power_vcc must drive 3-input VCC on AWG CH1 LOAD INF")
    delta_src = _fn_src(src, "_run_delta_icc")
    if "delta_icc_vectors" not in delta_src:
...
def _threshold_search_ok() -> list[str]:
    ...
    if not effective_search({}) or effective_search({}).get("step_ladder_V") != list(
        DEFAULT_SEARCH["step_ladder_V"]
    ):
        errors.append("missing recipe.search must use DEFAULT_SEARCH (never linear 0..VCC)")
    if effective_search({"search": {"vih": {"arm": 0.0}}}).get("vih", {}).get("arm") != 0.0:
        errors.append("YAML recipe.search must win over DEFAULT_SEARCH")
    sweep_src = _fn_src(_LOGIC_DC.read_text(encoding="utf-8"), "_sweep_threshold")
    if "effective_search" not in sweep_src:
        errors.append("_sweep_threshold must always use effective_search")
    if "for i in seq" in sweep_src or "threshold_step_v" in sweep_src:
        errors.append("_sweep_threshold must not keep a linear 0..VCC walk")
    ...
    if m14 is None:
        errors.append("rs1g14 product_model missing for Schmitt edge_limit")
    else:
        lim165 = lookup_vcc_grid_limits(m14, 1.65) or {}
        vt_lo = edge_limit(lim165, rising=True, schmitt=True)
        if vt_lo is None or abs(float(vt_lo) - 0.75) > 1e-12:
            errors.append(
                f"Schmitt first_step must use VT+ lo 0.75, got {vt_lo} (not VIH_min_V=None -> 0.01)"
            )
        if abs(first_step(ladder, vt_lo) - 0.5) > 1e-12:
            errors.append(f"Schmitt |VT+| 0.75 first_step must be 0.5, got {first_step(ladder, vt_lo)}")
    blob = dict(search)
...
def _scale_and_overlay_ok() -> list[str]:
    ...
    d12 = delta_icc_vectors(["A", "B", "C"])
    if len(d12) != 12:
        errors.append(f"rs1g97 delta_icc must be 3x4=12, not SeeLim 6 / ICC 8, got {len(d12)}")
    want12 = [
        {"near": "A", "others": {"B": "H", "C": "H"}},
        {"near": "A", "others": {"B": "H", "C": "L"}},
        {"near": "A", "others": {"B": "L", "C": "H"}},
        {"near": "A", "others": {"B": "L", "C": "L"}},
        {"near": "B", "others": {"A": "H", "C": "H"}},
        {"near": "B", "others": {"A": "H", "C": "L"}},
        {"near": "B", "others": {"A": "L", "C": "H"}},
        {"near": "B", "others": {"A": "L", "C": "L"}},
        {"near": "C", "others": {"A": "H", "B": "H"}},
        {"near": "C", "others": {"A": "H", "B": "L"}},
        {"near": "C", "others": {"A": "L", "B": "H"}},
        {"near": "C", "others": {"A": "L", "B": "L"}},
    ]
    if d12 != want12:
        errors.append(f"delta_icc 12-seq A/B/C MSB-down others mismatch, got {d12}")
    seelim6 = {
        ("A", (("B", "H"), ("C", "H"))),
        ("A", (("B", "L"), ("C", "L"))),
        ("B", (("A", "H"), ("C", "H"))),
    ...
    got_keys = {(x["near"], tuple(sorted(x["others"].items()))) for x in d12}
    if not seelim6 <= got_keys:
        errors.append("delta_icc 12-seq must include SeeLim all-H and all-L bookends")
    d2 = delta_icc_vectors(["A", "B"])
    if len(d2) != 4:
        errors.append(f"n=2 delta_icc must stay 2x2=4, got {len(d2)}")
    d97 = current_test_drives(m97)
    if (
        d97.get("A") is None
        or d97["A"].src != "psu"
        or d97["A"].ch != 1
        ...
        errors.append(f"rs1g97 current-test drives must be VA=PSU1 VB=PSU2 VC=AWG2, got {d97}")
    if not current_test_vcc_on_awg(m97):
        errors.append("rs1g97 3-input current tests must put VCC on AWG CH1")
    if current_test_vcc_on_awg(m08):
        errors.append("rs1g08 2-input must keep PSU CH1 as VCC (not AWG)")
    if p126["n"] != 4:
...
def _handoff_ok() -> list[str]:
    ...
    if (
        ...
        and "checklist = None" not in runner_src
        and "checklist: list | None = None" not in runner_src
    ):
...
def _confirmed_sim_sweep_ok() -> list[str]:
    ...
    def _apply_pin(instr, drive, volts, ilim, *, allow_psu_ch1=False):
        ...
        return orig_apply_pin(instr, drive, volts, ilim, allow_psu_ch1=allow_psu_ch1)

    def _apply_levels(instr, model, levels, vcc, ilim, *, drives=None, allow_psu_ch1=False):
        dmap = drives or model.pin_drive
        bench.bind(model, dmap, vcc)
        return orig_apply_levels(
            instr, model, levels, vcc, ilim, drives=drives, allow_psu_ch1=allow_psu_ch1
        )

    def _power_vcc(instr, vcc, ilim, *, vcc_awg=False):
        bench.vcc = float(vcc)
        bench.psu_v[1] = float(vcc)
        return orig_power_vcc(instr, vcc, ilim, vcc_awg=vcc_awg)
...
def _all_truth_coverage_ok() -> list[str]:
    """Every combinational Path B table is 2^n. dICC is n*2^(n-1). Never SeeLim 6."""
    from ate.fixture.modes import enabled_tests_for_part as _en

    errors: list[str] = []
    keys: list[str] = []
    for path in sorted(PARTS_DIR.glob("*.yaml")):
        key = path.stem.lower()
        if key in _PATH_A_LOGIC or key in _WAIT_SAMPLE_LOGIC:
            continue
        if not has_product_model(key):
            continue
        keys.append(key)
    if "rs1g08" not in keys or "rs1g97" not in keys:
        errors.append("truth coverage must include rs1g08 and rs1g97")
    m08 = load_product_model("rs1g08")
    if m08 is not None:
        a08 = isolation_for_run(m08, "A")
        if len(a08) != 1 or a08[0].fix.get("B") != "H" or a08[0].y_expect != "track":
            errors.append(f"rs1g08 isolation_for_run A must stay other=H track only, got {a08}")
    m86 = load_product_model("rs1g86")
    if m86 is not None:
        a86 = isolation_for(m86, "A")
        if not any(p.y_expect == "track" and p.fix.get("B") == "L" for p in a86):
            errors.append("rs1g86 XOR isolation A must record track other=L")
        if not any(p.y_expect == "invert" and p.fix.get("B") == "H" for p in a86):
            errors.append("rs1g86 XOR isolation A must record invert other=H (not drop from table)")
    for key in keys:
        m = load_product_model(key)
        if m is None:
            errors.append(f"{key} product_model failed to load")
            continue
        if is_sequential(m) or is_parked(m):
            plan = sim_icc_plan(m)
            if int(plan.get("n") or 0) != 0:
                errors.append(f"{key} sequential/parked ICC must stay 0 corners, got {plan.get('n')}")
            continue
        holes = missing_truth_vectors(m)
        if (m.truth_table or []) and holes:
            errors.append(
                f"{key} truth_table missing 2^n combos (SeeLim-6 class): {holes[:8]}"
            )
        for pin in m.logic_inputs:
            if not isolation_for_run(m, pin) and (m.truth_table or []):
                errors.append(f"{key} no isolation_for_run {pin} from truth_table")
        plan = sim_icc_plan(m)
        n_pin = len(plan.get("pins") or [])
        n_corner = int(plan.get("n") or 0)
        if n_pin and n_corner != (1 << n_pin):
            errors.append(
                f"{key} ICC corners must be 2^{n_pin}={1 << n_pin}, got {n_corner}"
            )
        en = {str(x).strip().lower() for x in (_en(key) or [])}
        if "delta_icc" in en:
            pins = delta_icc_pins(m)
            n = len(pins)
            want = n * (1 << (n - 1)) if n else 0
            got = len(delta_icc_vectors(pins))
            if got != want:
                errors.append(
                    f"{key} delta_icc must be {n}*2^{n - 1}={want} (not SeeLim 6 / ICC {1 << n}), got {got}"
                )
        owned = vcc_grid_owned(m.vcc_grid) if m.vcc_grid else {}
        if owned and "input_threshold" in en and not vcc_grid_unconfirmed(m):
            for vcc, row in owned.items():
                lim = lookup_vcc_grid_limits(m, vcc) or row
                if m.schmitt:
                    if not (lim.get("VT_plus") or lim.get("VT+") or lim.get("VIH_min_V")):
                        errors.append(
                            f"{key} vcc_grid @{vcc} missing VT+/VIH (datasheet limit)"
                        )
                    if not (lim.get("VT_minus") or lim.get("VT-") or lim.get("VIL_max_V")):
                        errors.append(
                            f"{key} vcc_grid @{vcc} missing VT-/VIL (datasheet limit)"
                        )
                else:
                    if lim.get("VIH_min_V") is None and lim.get("VIH_min") is None:
                        errors.append(f"{key} vcc_grid @{vcc} missing VIH_min_V")
                    if lim.get("VIL_max_V") is None and lim.get("VIL_max") is None:
                        errors.append(f"{key} vcc_grid @{vcc} missing VIL_max_V")
    return errors
