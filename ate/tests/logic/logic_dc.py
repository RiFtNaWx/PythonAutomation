...
INSTRUMENT_SENSE ICC: DMM-on-VCC. n<3: PSU CH1=VCC, inputs from pin_drive.
  n>=3: VCC=AWG CH1 (LOAD INF); VA=PSU CH1 VB=PSU CH2 VC=PSU CH3.
INSTRUMENT_SENSE DELTA_ICC: DMM-on-VCC; one input at recipe.delta_offset_v
  ...
  Always n * 2^(n-1) others (12 for A/B/C). Never SeeLim 6 bookends.
  DP832 CH3 ceiling is 5 V -- C at 5.5 V clamps to 5.0 so CH3 still ON. VIH YAML pin_drive stays.
...
def _needs_awg(model: ProductModel) -> bool:
    if current_test_vcc_on_awg(model):
        return True
...
def _sim_is(instr) -> bool:
    if getattr(instr, "_simulated", False):
        return True
    dmm = getattr(instr, "dmm", None)
    return bool(getattr(dmm, "simulated", False))


def _sim_bus_begin(instr, model: ProductModel, part_key: str) -> None:
    if not _sim_is(instr):
        return
    from ate.instruments.sim import set_open_drain, set_schmitt, set_y_invert

    cfg = load_part_yaml(part_key)
    runner_cls = str(
        (model.raw or {}).get("product_class")
        or model.recipe.get("runner")
        or ""
    ).lower()
    if "y_invert" in cfg:
        set_y_invert(bool(cfg.get("y_invert")))
    elif "inv" in runner_cls:
        set_y_invert(True)
    set_schmitt(bool(model.schmitt))
    set_open_drain(is_open_drain(model))


def _sim_bus_end(instr) -> None:
    if not _sim_is(instr):
        return
    from ate.instruments.sim import set_logic_loaded, set_open_drain, set_schmitt, set_y_invert

    set_logic_loaded(None)
    set_open_drain(False)
    set_schmitt(False)
    set_y_invert(False)


def _sim_loaded(instr, mode: str | None) -> None:
    if not _sim_is(instr):
        return
    from ate.instruments.sim import set_logic_loaded

    set_logic_loaded(mode)
...
def _vcc_corners(params: Any, model: ProductModel, recipe_key: str = "") -> list[float]:
    ...
    if isinstance(overlay, list) and overlay:
        return [float(x) for x in overlay]
...
def _power_vcc(instr, vcc: float, ilim: float, *, vcc_awg: bool = False) -> None:
    if vcc_awg:
        from generator_setup import set_output_load, setup_dc
        ...
        if not setup_dc(instr.gen, 1, float(vcc)):
            raise RuntimeError(f"AWG CH1 VCC DC setup failed at {vcc} V")
        return
...
def _force_psu(instr, ch: int, volts: float, ilim: float) -> None:
    ...
    if int(ch) == 3 and v > 5.0:
        ...
        v = 5.0
        ovp = 5.3
    power_on_protected(instr.psu, ch, v, ilim, ovp=ovp)
...
def _apply_pin(
    instr, drive: DriveMap, volts: float, ilim: float, *, allow_psu_ch1: bool = False
) -> None:
    if drive.src == "awg":
...
def _apply_levels(
    ...
    *,
    ...
    for pin, lv in levels.items():
        ...
        _apply_pin(
            instr, drive, logic_volts(lv, vcc), ilim, allow_psu_ch1=allow_psu_ch1
        )
...
def _sweep_threshold(
    ...
    *,
    ...
    from ate.tests.logic.threshold_search import edge_limit, effective_search, run_search_stage
    ...
    search = effective_search(model.recipe)
    lim = lookup_vcc_grid_limits(model, vcc) or {}
    limit_f = edge_limit(lim, rising=rising, schmitt=bool(model.schmitt))
    ...
    with path_b_handoff(params, model, "delta_icc"):
        ...
        try:
            for vcc in vccs:
                ...
                plan: list[dict[str, Any]] = delta_icc_vectors(pins)
                for item in plan:
                    near = str(item["near"])
                    levels = dict(item.get("others") or {})
                    if model.has_oe() and model.oe_pin not in levels and near != model.oe_pin:
                    ...
                    drive = drives.get(near) or model.pin_drive.get(near)
                    if drive is None:
                        raise RuntimeError(f"delta_icc: no pin_drive for {near}")
                    _apply_pin(instr, drive, near_v, ilim, allow_psu_ch1=vcc_awg)
                    i_ua = _wait_settled_current_ua(instr.dmm, model, setup=False)
                    others_tag = ",".join(
                        f"{k}={v}" for k, v in sorted((levels or {}).items())
                    )
                    rows.append(
                        {
                            "VCC": vcc,
...
def _run_voh_path_b(instr, params: Any) -> dict[str, Any]:
    ...
    _sim_bus_begin(instr, model, key)
    try:
                ...
                if table:
                    ...
                    for entry in table:
                            ...
                            rows.append(
                                {
                                    "VCC": vcc,
                                    "status": "PROVISIONAL",
                                    "note": "voh_table row missing spec/ioh",
                                }
                            )
                            continue
                        vref = entry.get("vref")
                        ...
                        power_on_protected(
                            ...
                            float(vref),
                            abs(float(ioh)),
                            ocp=max(0.05, abs(float(ioh)) * 1.2),
                        )
                        _sim_loaded(instr, "voh_sink")
                        try:
                            measured = _wait_settled_voltage(instr.dmm, model, setup=False)
                        finally:
                            _sim_loaded(instr, None)
                ...
                if table:
                    ...
                    for entry in table:
                            ...
                            rows.append(
                                {
                                    "VCC": vcc,
                                    "status": "PROVISIONAL",
                                    "note": "vol_table row missing spec/iol",
                                }
                            )
                            continue
                        vref = entry.get("vref")
                        ...
                        power_on_protected(
                            ...
                            float(vref),
                            abs(float(iol)),
                            ocp=max(0.05, abs(float(iol)) * 1.2),
                        )
                        if vplus is not None:
                            power_on_protected(instr.psu, 3, float(vplus), ilim)
                        _sim_loaded(instr, "vol_source")
                        try:
                            measured = _wait_settled_voltage(instr.dmm, model, setup=False)
                        finally:
                            _sim_loaded(instr, None)
