...
def iter_logic_corners_msb_down(pins: Iterable[str]) -> list[dict[str, str]]:
    """First pin is MSB; count 2^n-1 .. 0. 3-in: HHH HHL HLH HLL LHH LHL LLH LLL."""
    names = [str(p).strip().upper() for p in pins if str(p).strip()]
    n = len(names)
    if not n:
        return [{}]
    ...
    for mask in range((1 << n) - 1, -1, -1):
        row = {
            names[i]: ("H" if (mask >> (n - 1 - i)) & 1 else "L") for i in range(n)
        }
        out.append(row)
    return out


def current_test_vcc_on_awg(model: ProductModel) -> bool:
    """n>=3 ICC/dICC: VCC on AWG CH1. C is AWG CH2 (DP832 CH3 is 5 V max)."""
    return len(list(model.logic_inputs or [])) >= 3


def current_test_drives(model: ProductModel) -> dict[str, DriveMap]:
    """n>=3 current tests: VA=PSU1 VB=PSU2 VC=AWG2. VCC=AWG1. VIH YAML stays."""
    pins = [str(p).strip().upper() for p in (model.logic_inputs or []) if str(p).strip()]
    if len(pins) < 3:
        return dict(model.pin_drive)
    return {
        pins[0]: DriveMap(src="psu", ch=1),
        pins[1]: DriveMap(src="psu", ch=2),
        pins[2]: DriveMap(src="awg", ch=2),
    }


def current_test_corners(model: ProductModel) -> list[dict[str, str]]:
    pins = icc_pins(model)
    if len(pins) >= 3:
        return iter_logic_corners_msb_down(pins)
    return iter_logic_corners(pins)


def delta_icc_vectors(pins: Iterable[str]) -> list[dict[str, Any]]:
    """One pin biased; others 2^(n-1) MSB-down. n=3 -> 12 (not SeeLim 6, not ICC 8)."""
    names = [str(p).strip().upper() for p in pins if str(p).strip()]
    out: list[dict[str, Any]] = []
    for near in names:
        others = [p for p in names if p != near]
        for vec in iter_logic_corners_msb_down(others):
            out.append({"near": near, "others": dict(vec)})
    return out
...
def isolation_for_run(model: ProductModel, sweep_pin: str) -> list[IsolationPattern]:
    """One unused-tie hold per pin: first track, else first invert.

    Full truth_table still records every track/invert hold (SeeLim-6 lives in
    dICC / missing 2^n rows, not extra VIH walks). Skip PROPOSED / HOLD CONFIRM.
    """
...
def truth_space_pins(model: ProductModel) -> list[str]:
    """Inputs that span the combinational table: logic_inputs, plus OE if listed."""
    pins = [str(p).strip().upper() for p in (model.logic_inputs or []) if str(p).strip()]
    oe = str(model.oe_pin or "").strip().upper()
    if model.has_oe() and oe:
        if any(oe in row for row in (model.truth_table or [])):
            if oe not in pins:
                pins.append(oe)
    return pins


def _expand_truth_row(row: dict[str, str], pins: list[str]) -> list[dict[str, str]]:
    combos: list[dict[str, str]] = [{}]
    for pin in pins:
        cell = _norm_level(row.get(pin, "X"))
        nxt: list[dict[str, str]] = []
        for cur in combos:
            if cell == "X":
                nxt.append({**cur, pin: "L"})
                nxt.append({**cur, pin: "H"})
            elif cell in ("H", "L"):
                nxt.append({**cur, pin: cell})
            else:
                nxt.append({**cur, pin: cell})
        combos = nxt
    return combos
...
def covered_truth_keys(model: ProductModel) -> set[tuple[str, ...]]:
    """2^n H/L keys that have a defined Y (H/L/Z). X in the table expands."""
    pins = truth_space_pins(model)
    if not pins:
        return set()
    out: set[tuple[str, ...]] = set()
    for row in model.truth_table or []:
        y = _norm_level(row.get(model.output_pin))
        if y in ("", "X") or y is None:
            continue
        for combo in _expand_truth_row(row, pins):
            if all(combo.get(p) in ("H", "L") for p in pins):
                out.add(tuple(combo[p] for p in pins))
    return out


def expected_truth_keys(model: ProductModel) -> set[tuple[str, ...]]:
    pins = truth_space_pins(model)
    n = len(pins)
    if not n:
        return set()
    return {
        tuple("H" if (mask >> i) & 1 else "L" for i in range(n))
        for mask in range(1 << n)
    }


def missing_truth_vectors(model: ProductModel) -> list[dict[str, str]]:
    """Combinational holes vs 2^n. Sequential is not this check."""
    if is_sequential(model):
        return []
    pins = truth_space_pins(model)
    missing = expected_truth_keys(model) - covered_truth_keys(model)
    out: list[dict[str, str]] = []
    for key in sorted(missing):
        out.append({pins[i]: key[i] for i in range(len(pins))})
    return out


def delta_icc_pins(model: ProductModel) -> list[str]:
    """dICC pin list: data, plus OE when present. Same as _run_delta_icc."""
    pins = [str(p).strip().upper() for p in (model.logic_inputs or []) if str(p).strip()]
    if model.has_oe() and model.oe_pin and model.oe_pin not in pins:
        pins.append(str(model.oe_pin).strip().upper())
    return pins
...
def sim_icc_plan(model: ProductModel) -> dict[str, Any]:
    ...
    pins = icc_pins(model)
    corners = current_test_corners(model)
...
def format_stimulus_lines(model: ProductModel, test_id: str) -> list[str]:
    ...
    if tid == "voh":
        ...
        if str(model.part or "").strip().upper() == "RS1GT34":
            lines.append(
                "VOL board-change: recable PSU CH2 Y-load; do not invent VOL"
            )
    ...
    if dual_channel_continue(model):
        lines.append(
            "Do not skip rewire CHA then CHB. Recable Channel B after CHA Human Continue."
        )
    return lines
