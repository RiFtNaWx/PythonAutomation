...
Do not invent a reverse sweep. Schmitt VT+/VT- use the same helper.
Missing recipe.search -> DEFAULT_SEARCH (never a 0..VCC linear walk).
"""
...
# Runtime default when YAML omits recipe.search. Do not copy this blob per SKU.
DEFAULT_SEARCH: dict[str, Any] = {
    ...
    "step_ladder_V": list(DEFAULT_LADDER),
    "on_hit": {
        "skip_rest_of_walk": True,
        "rearm": True,
        "restart_fraction_back": 0.3,
        "next_smaller_step": True,
    },
    "after_precision": {"interpolate": True},
}
...
def effective_search(recipe: Any) -> dict[str, Any]:
    """YAML search if present, else DEFAULT_SEARCH. Never an empty linear walk."""
    blob = search_blob(recipe)
    return blob if blob else dict(DEFAULT_SEARCH)


def _band_lo(raw: Any) -> Optional[float]:
    if isinstance(raw, (list, tuple)) and raw:
        try:
            return abs(float(raw[0]))
...
def edge_limit(
    ...
    *,
    ...
    """Card |limit| for first_step. Schmitt uses VT+/VT- lo, else VIH_min / VIL_max."""
    row = grid_row if isinstance(grid_row, dict) else {}
    if schmitt:
        if rising:
            lo = _band_lo(row.get("VT_plus") or row.get("VT+") or row.get("VTPLUS"))
        else:
            lo = _band_lo(row.get("VT_minus") or row.get("VT-") or row.get("VTMINUS"))
        if lo is not None:
            return lo
    key = "VIH_min_V" if rising else "VIL_max_V"
    alt = "VIH_min" if rising else "VIL_max"
    raw = row.get(key, row.get(alt))
    return _band_lo(raw)
