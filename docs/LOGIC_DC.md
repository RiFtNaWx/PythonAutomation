...
Operator bench (DEMO/SIM vs HUMAN+instruments, Excel/log paths, 97/126 checklist): `docs/LOGIC_DC_OPERATOR.md`. AE/FAE add-part flow: `docs/LOGIC_DC_HANDOVER.md`. Dual-channel Continue (future 2Gxx): `docs/LOGIC_DC_DUAL_CHANNEL.md`. DEMO/SIM and `check_logic_dc` are **not** a reproduce claim.
...
product_model:
  ...
  # threshold_isolation: [{sweep, hold, y_tracks, status}]
    ...
    search:               # optional; missing uses DEFAULT_SEARCH coarse-to-fine
...
- **VIH/VIL (or VT+/VT-)** -- unused ties from the truth table. Prefer a combo where **Y tracks the swept pin non-inverting**. Invert only when no track combo exists. Derived isolation still lists every track/invert hold (XOR dual; mux C invert stays in the table). `isolation_for_run` skips `PROPOSED` / `HOLD CONFIRM`. Per-VCC VIH min / VIL max come from `vcc_grid`. Range steps inherit band limits -- never a fixed-point row. `vcc_grid.status` UNCONFIRMED is not greenable. `threshold_search.py` always walks VIH up / VIL down: limit-scaled first step, on-hit skip rest, no reverse. Missing `recipe.search` uses DEFAULT_SEARCH (never a 0..VCC linear walk). Schmitt first step uses VT+/VT- lo.
- **II** -- per input, VI=0 and VI=max.
- **Delta ICC** -- one input at VCC-offset, or at `dc_limits.ICCT_uA.one_input_V` when ICCT is mapped. Walk `n * 2^(n-1)` others (12 for A/B/C). Never SeeLim 6 all-H/all-L bookends. RS1GT34 enables `delta_icc` from ICCT (500uA @5.5V one_in@3.4). Do not invent `delta_offset_v=0.6`. CMOS cards may map `ICCT_uA.offset_v` (on the card, not invented).
